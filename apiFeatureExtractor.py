...
class APIfeatureExtractor():
    def __init__(self):
        self.jawOpen = feature.Feature(scaleType = 2, curve = 2,  spring = 1, friction = 0.5, mass = 2)
        self.mouthX = feature.Feature(curve = 1, scaleType = 1,  spring = 0.5, friction = 0.5, mass = 2)
        self.mouthOpen = feature.Feature(scaleType = 2, curve = 0.5, decay=0.0001,  spring = 0.75, friction = 0.5, mass = 1)
        self.mouthSmile= feature.Feature(curve = 2,  spring = 0.75, friction = 0.5, mass = 1)
        self.mouthPucker = feature.Feature(curve = 2,  spring = 0.75, friction = 0.5, mass = 1)
        ...
        self.eyeX = feature.Feature(scaleType = 1,  spring = 0.9, friction = 0.5, mass = 1)
        self.eyeY = feature.Feature(scaleType = 1,  spring = 0.9, friction = 0.5, mass = 1)
        ...
        self.faceAngleZ = feature.Feature( curve = 1, scaleType = 1, spring = 0.33, friction = 0.66, mass = 5)
        ...
        self.facePositionZ = feature.Feature( curve = 1, scaleType = 1, spring = 0.33, friction = 0.33, mass = 3)
    ...
    def update(self, pts, rotation, facePosition, confidence):
        ...
        eyeLeft = self.eyeOpenLeft.update(f, calibrate)

        f = maffs.euclideanDistance(maffs.average3d(pts[[36,39]]), maffs.average3d(pts[[37,38]]))
        eyeRight = self.eyeOpenLeft.update(f, calibrate)
        ...
        f = rotation[0]
        xAxis =  30 * self.faceAngleX.update(f, calibrate)
        features.append(["FaceAngleX",xAxis])

        f = -rotation[1]
        features.append(["FaceAngleY", 20 * self.faceAngleY.update(f, calibrate)])

        f = rotation[2]
        features.append(["FaceAngleZ", (30 * self.faceAngleZ.update(f, calibrate)) ])

        f = facePosition[0]
        features.append(["FacePositionX", -self.facePositionX.update(f, calibrate)*5])

        f = facePosition[1]
        features.append(["FacePositionY", -self.facePositionY.update(f, calibrate)*5])

        f = facePosition[2]
        features.append(["FacePositionZ", -self.facePositionZ.update(f, calibrate)*5])
